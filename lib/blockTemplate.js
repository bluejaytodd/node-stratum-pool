...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, extraNoncePlaceholder, reward, recipients, poolAddress){
    ...
    this.serializeHeader = function(nTime, nonce){
        ...
        console.log('this.rpcData.height: ' + this.rpcData.height);
    ...
    this.serializeBlock = function(header, soln){
        ...
        tmptoddbuf = new Buffer.from(this.genTx, 'hex');
        tmptoddbuf2 = Buffer.alloc(tmptoddbuff.length-9);
        tmptoddbuf3 = new  Buffer('01000000','hex');
        tmptoddbuf3.copy(tmptobuf2,0,0,4);
        tmptoddbuf.copy(tmptobuf2,4,8,tmptoddbuf.length-13);
        
        buf = new Buffer.concat([
            header,
            soln,
            varInt,
            new Buffer(tmptoddbuf2, 'hex')
        ]);

        // todd jjkk 
        console.log('--------------------------------------------------------');
        console.log('buf_Tx: ' + tmptoddbuf2.toString('hex'));
